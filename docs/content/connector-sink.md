...
- Configurations to guarantee exactly-once
  ...
  - Optionally tune the PREPARED timeout with `sink.properties.prepared_timeout` (supported since 1.2.12). In exactly-once, if not set, the timeout falls back to StarRocks FE `prepared_transaction_default_timeout_second` (default 86400s). See [Transaction timeout management](https://docs.starrocks.io/docs/loading/Stream_Load_transaction_interface/#transaction-timeout-management).
