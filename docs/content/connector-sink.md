...
| sink.sanitize-error-log          | No           | false             | When true, sensitive row data and column values in Stream-Load error logs are redacted in both connector and SDK logs. Intended for production security; defaults to false for backward compatibility. |
