...
| sink.label-prefix                 | No           | flink             | The label prefix used by Stream Load. Recommend to configure it if you are using exactly-once with connector 1.2.8 and later. See [exactly-once usage notes](#exactly-once).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| sink.sanitize-error-log          | No           | false             |  Supported since 1.2.12. When true, sensitive row data and column values in Stream-Load error logs are redacted in both connector and SDK logs. Intended for production security; defaults to false for backward compatibility. |
...
### Letting server side DEFAULTs fire with the json format

With the `json` format the connector sends no `columns` header by default. The server then declares
every table column for the load, and a column the payload has no key for is stored as an explicit
NULL, which overrides any DEFAULT that column declares. A column such as `ingest_ts DATETIME DEFAULT
CURRENT_TIMESTAMP` therefore lands empty unless every row sends it.

Set `sink.json.columns-from-flink-schema=true` and the connector sends the Flink table's columns
as the header instead, the way it already does for `csv` when the Flink and StarRocks schemas
differ. A column the Flink table does not declare is then left off the header, so the server
applies its DEFAULT. This means the Flink table definition is the column list: leave a defaulted
