...
public class StarRocksSinkOptions implements Serializable {
    ...
    public static final ConfigOption<Boolean> SINK_WITH_NO_SCHEMA = ConfigOptions.key("sink.with-no-schema")
            .booleanType().defaultValue(false).withDescription(" stream elements could only be String");
    ...
    public Boolean getSinkWithNoSchema() {
        return tableOptions.getOptional(SINK_WITH_NO_SCHEMA).orElse(false);
    }
    ...
    private void validateSinkModel() {
        tableOptions.getOptional(SINK_WITH_NO_SCHEMA).ifPresent(val -> {
            if (val) {
                Map<String, String> loadProsp = getSinkStreamLoadProperties();
                String format = loadProsp.get(FORMAT_KEY);
                if (null != format && !StreamLoadFormat.JSON.name().equalsIgnoreCase(format)) {
                    throw new ValidationException(
                            String.format("Format must be JSON When sink with no schema.Current format is '%s'",
                                    format));
                }
            }
        });
