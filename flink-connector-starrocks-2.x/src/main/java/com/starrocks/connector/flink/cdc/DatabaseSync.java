...
public abstract class DatabaseSync {
    ...
    public void build() throws Exception {
        ...
        for (String table : starRocksTables) {
            ...
            Sink<String> starRockSink = StarRocksSink.sink(starRocksSinkOptions);
            sideOutput.map(new DebeziumJsonProcess(getSerializer(table))).filter(new DebeziumJsonFilter()).sinkTo(starRockSink).setParallelism(sinkParallel).name(table);
        }
