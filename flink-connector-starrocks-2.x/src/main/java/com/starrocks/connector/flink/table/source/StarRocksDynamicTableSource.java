...
public class StarRocksDynamicTableSource implements ScanTableSource, LookupTableSource, SupportsFilterPushDown, SupportsProjectionPushDown {
    ...
    public StarRocksDynamicTableSource(StarRocksSourceOptions options, ResolvedSchema schema, PushDownHolder pushDownHolder) {
    ...
    public ScanRuntimeProvider getScanRuntimeProvider(ScanContext scanContext) {
        StarRocksFlinkSource starRocksSource = new StarRocksFlinkSource(
            options, flinkSchema,
        ...
        return SourceProvider.of(starRocksSource);
    }
    ...
    public LookupRuntimeProvider getLookupRuntimeProvider(LookupContext context) {
        ...
        for (int i = 0; i < projectedFields.length; i ++) {
            ColumnRichInfo columnRichInfo = new ColumnRichInfo(
                this.flinkSchema.getColumn(projectedFields[i]).get().getName(), 
                projectedFields[i],
                this.flinkSchema.getColumn(projectedFields[i]).get().getDataType()
            );
        ...
        StarRocksDynamicLookupFunction lookupFunction = new StarRocksDynamicLookupFunction(this.options, filerRichInfo, ColumnRichInfos, selectColumns);
        return LookupFunctionProvider.of(lookupFunction);
    }

    /**
     * NOTE: this function need deep copy according to the comment of {@link DynamicTableSource#copy()}
     * ResolvedSchema is immutable in Flink 2.0+ so no copy needed.
     */
    @Override
    public DynamicTableSource copy() {
        StarRocksSourceOptions options = this.options.copy();
        PushDownHolder pushDownHolder = this.pushDownHolder.copy();
        return new StarRocksDynamicTableSource(options, this.flinkSchema, pushDownHolder);
    }
    ...
    public void applyProjection(int[][] projectedFields, DataType producedDataType) {
        ...
        for (int index : curProjectedFields) {
            String columnName = flinkSchema.getColumn(index).get().getName();
