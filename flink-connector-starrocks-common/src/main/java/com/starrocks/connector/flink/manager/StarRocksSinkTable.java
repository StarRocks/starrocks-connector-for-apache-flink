...
public class StarRocksSinkTable {
    ...
    public void validateTableStructure(StarRocksSinkOptions sinkOptions, ResolvedSchema flinkSchema) {
        if (flinkSchema == null) {
            return;
        }
        ...
        for (Column column : flinkSchema.getColumns()) {
            ...
            boolean typeMatched = !typesMap.containsKey(srType) || typesMap.get(srType).contains(column.getDataType().getLogicalType().getTypeRoot());
            if (!typeMatched) {
                throw new IllegalArgumentException(
                        String.format("Flink and StarRocks types are not matched for column %s, " +
                                "flink type is %s, starrocks type is %s", column.getName(), column.getDataType(), srType));
            }
            columnPositionInStarRocksSchema.add((long) srColumn.get("ORDINAL_POSITION"));
        }
        sinkOptions.setTableSchemaFieldNames(flinkSchema.getColumnNames().toArray(new String[0]));
        ...
        if (!primaryKeys.isEmpty()) {
            ...
            flinkAndStarRocksSchemaAligned = flinkSchema.getColumns().size() == starrocksColumnMapping.size() &&
