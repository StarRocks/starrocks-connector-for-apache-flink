...
public class StarRocksSinkOptions implements Serializable {
    ...
    public static final ConfigOption<Boolean> SINK_USE_NEW_SINK_API = ConfigOptions.key("sink.use.new-sink-api")
            .booleanType().defaultValue(false).withDescription("Whether to use the implementation with the unified sink api " +
                    "described in Flink FLIP-191. When not set explicitly, it defaults to false on Flink 1.x (where the " +
                    "legacy SinkFunction api still exists) and to true on Flink 2.x (where the legacy api was removed, " +
                    "making the unified sink the only choice). Note that it's not compatible after changing the flag, " +
                    "that's, you can't recover from the previous job after changing the flag.");
    ...
    public boolean isUseUnifiedSinkApi() {
        return tableOptions.getOptional(SINK_USE_NEW_SINK_API)
                .orElse(!LEGACY_SINK_FUNCTION_API_AVAILABLE);
    }

    /**
     * The legacy SinkFunction api was removed in Flink 2.0; its absence flips the
     * effective default of {@link #SINK_USE_NEW_SINK_API} to true.
     */
    private static final boolean LEGACY_SINK_FUNCTION_API_AVAILABLE = detectLegacySinkFunctionApi();

    private static boolean detectLegacySinkFunctionApi() {
        try {
            Class.forName("org.apache.flink.streaming.api.functions.sink.SinkFunction",
                    false, StarRocksSinkOptions.class.getClassLoader());
            return true;
    ...
    private void validateMultiTableTransaction() {
        ...
        if ("V1".equalsIgnoreCase(tableOptions.get(SINK_VERSION))) {
            throw new ValidationException(
